...
let setPlotlyTheme = (theme) => {
  document.querySelectorAll(".js-plotly-plot").forEach((elem) => {
    ...
    if (theme === "dark") {
      Plotly.relayout(elem, { template: "plotly_dark" });
    } else {
      Plotly.relayout(elem, { template: "plotly_white" });
    }
  });
};
...
// "system". Default is "light" (local override: the site should not silently follow the
// visitor's OS dark-mode preference on first visit; a returning visitor's explicit choice,
// stored in localStorage, is always respected).
